...
from random import expovariate as exp, uniform, choice, random
...
approve_queue= loan_process.add_var("approve_queue")
reject_queue = loan_process.add_var("reject_queue")
...
def final_decision(token):
    if random() < 0.5:
        return [SimToken(token), None]  # Approve
    else:
        return [None, SimToken(token)]  # Reject

loan_process.add_event(
    [join_done],
    [approve_queue, reject_queue],
    final_decision,
    name="approve_or_reject"
)

BPMNEndEvent(loan_process, [approve_queue], [], "application_approved")
BPMNEndEvent(loan_process, [reject_queue],  [], "application_rejected")
